package com.atguigu.gmall.index.uitils;
...
public class DistribuitedLock {
    ...
    private void renewExpire(String lockName, String uuid, Integer expire) {
        ...
        new Timer().schedule(new TimerTask() {
            @Override
            public void run() {
                Boolean execute = redisTemplate.execute(new DefaultRedisScript<>(script, Boolean.class), Arrays.asList(lockName), uuid, expire);
                // 如果这一次续期成功,则开启下一个周期的续期
                if (execute) {
...
//            System.out.println("这是juc中定时任务：" + System.currentTimeMillis());
